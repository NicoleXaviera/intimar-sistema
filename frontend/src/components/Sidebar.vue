<template>
  ...
  <!-- Top Bar para Móvil (Logo y Perfil) -->
  <div class="md:hidden fixed top-0 left-0 right-0 h-16 bg-white/80 backdrop-blur-md border-b border-gray-100 z-[1000] flex items-center justify-between px-6">
    ...
    <div class="relative">
      <button 
        @click="showProfileMenu = !showProfileMenu"
        class="w-10 h-10 rounded-full overflow-hidden border-2 border-intimar-primary/20 shadow-sm"
      >
        <img v-if="userImage" :src="userImage" class="w-full h-full object-cover" />
        <div v-else class="w-full h-full bg-intimar-primary flex items-center justify-center text-white text-[10px] font-black uppercase">
          {{ userInitials }}
        </div>
      </button>

      <!-- Menú Desplegable Mobile (Desde el Top) -->
      <div v-if="showProfileMenu" class="absolute right-0 mt-3 w-56 bg-white rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-gray-100 overflow-hidden animate-in fade-in slide-in-from-top-4 duration-300 z-[1001]">
        <div class="p-4 border-b border-gray-50 bg-gray-50/50">
          <p class="text-[9px] font-black text-gray-400 uppercase tracking-widest">Usuario</p>
          <p class="text-xs font-black text-intimar-dark truncate italic">{{ userFullName }}</p>
        </div>
        <div class="p-2 space-y-1">
          <router-link to="/perfil" @click="showProfileMenu = false" class="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 rounded-2xl transition-colors text-[11px] font-bold text-gray-600">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="text-intimar-primary"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
            Mi Perfil
          </router-link>
          <button @click="handleLogout" class="w-full flex items-center gap-3 px-4 py-3 hover:bg-red-50 rounded-2xl transition-colors text-[11px] font-bold text-red-600">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>
            Cerrar Sesión
          </button>
        </div>
  ...
  <div class="md:hidden fixed bottom-4 left-4 right-4 h-16 bg-intimar-primary rounded-full shadow-[0_15px_40px_rgba(0,147,143,0.3)] z-[1000] flex items-center justify-around px-2">
    <!-- Mapa -->
    <router-link to="/mapa" 
      class="flex flex-col items-center justify-center transition-all duration-300 w-11 h-11 rounded-2xl" 
