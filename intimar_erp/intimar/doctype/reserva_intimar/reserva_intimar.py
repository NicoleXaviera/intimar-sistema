...
class ReservaIntimar(Document):
			...
			if conflictos:
				...
				reservas_activas = frappe.get_all("Reserva Intimar",
					filters={
						...
						"estado_reserva": ["not in", ["Cancelada", "Finalizada", "Atrasada"]]
					},
					fields=["name"]
				)
				if reservas_activas:
	...
	def validate_aforo(self):
		...
		if self.estado_reserva in ["Cancelada", "Finalizada", "Lista de espera", "Atrasada"]:
			return
		...
		if not self.is_new():
			...
			if old_doc and old_doc.estado_reserva in ["Confirmada", "En proceso"]:
				...
				if pax_nuevos <= old_pax and str(old_doc.fecha_reserva) == str(self.fecha_reserva) and format_time(old_doc.hora_reserva, "HH:mm") == format_time(self.hora_reserva, "HH:mm"):
					return 
	...
	def handle_table_states(self):
		...
		if self.estado_reserva == "En proceso":
			disponible = 0
		elif self.estado_reserva in ["Finalizada", "Cancelada", "Atrasada"]:
			disponible = 1
		else:
			return
